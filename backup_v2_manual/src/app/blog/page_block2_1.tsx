...
export default function BlogPage() {
    ...
    useEffect(() => {
        ...
        if (savedGroups) {
            ...
            setNewProfileCategory(parsed[0]); // Update default select value
        }
    }, []);
    ...
    const handleBatchGenerate = async () => {
        ...
        // Sequential Processing (Safe Mode for Free Tier)
        // Process items one by one to strictly avoid "Quota Exceeded" / 429 errors.
        for (let i = 0; i < batchItems.length; i++) {
            const item = batchItems[i];

            // Skip invalid or already successful items (allows retrying only failed ones)
            if (!item.title || !item.keyword) continue;
            ...
            try {
                ...
                // If it's a critical error (like quota), maybe we should pause? 
                // For now, we continue, but the sequential delay helps recover.
            }

            // Safety Delay: Wait 2 seconds between requests to be extremely safe for Free Tier
    ...
    const handleSaveGroups = () => {
        ...
        setNewProfileCategory(tempCategories[0]); // Reset selection to first new group
                    ...
                    <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-scale-up">
                        ...
                        <div className="p-6 max-h-[60vh] overflow-y-auto custom-scrollbar">
                            <p className="text-xs text-gray-500 mb-4 bg-blue-50 p-3 rounded-lg leading-relaxed">
                                * 프로필을 분류할 10개의 그룹 이름을 자유롭게 변경하세요.<br />
                                (예: 클라이언트A, 개인블로그, 맛집탐방 등)
                            </p>
                                                ...
                                                <div className="w-20 flex justify-end flex-none">
                                                    ...
                                                    {item.status === 'error' && (
                                                        <div className="group/error relative">
                                                            <span className="text-xs text-red-500 font-bold cursor-help underline decoration-dashed">Error</span>
                                                            <div className="absolute right-0 top-full mt-1 w-48 bg-red-50 border border-red-200 text-red-600 text-[10px] p-2 rounded shadow-lg z-20 hidden group-hover/error:block">
                            ...
                            <div className="flex items-center justify-between mb-4 flex-none">
                                ...
                                <div className="flex items-center gap-2">
                                    <button
                                        ...
                                        className="text-xs font-bold text-gray-500 bg-gray-100 hover:bg-gray-200 px-2 py-1 rounded-lg transition-colors flex items-center gap-1"
                                        title="그룹(카테고리) 설정"
                                    >
