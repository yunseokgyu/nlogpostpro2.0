...
(async () => {
    try {
        ...
        // Convert Map to Array {id, name}
        // Data format: { "afghanistan": { ... }, "russia": { ... } }
        const countryList = Object.keys(countriesData).map(key => ({
            id: key,
            name: countriesData[key].text_en || key // Use English text or key
        }));

        // Sort alphabetically
        countryList.sort((a, b) => a.name.localeCompare(b.name));

        console.log(`Saving ${countryList.length} countries to countries_list.json...`);
        fs.writeFileSync('countries_list.json', JSON.stringify(countryList, null, 2), 'utf8');
        console.log('Done.');
