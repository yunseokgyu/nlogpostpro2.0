    ...
    const handleEmailLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (error) {
                ...
                setMessage("✅ 이메일로 로그인 링크가 전송되었습니다! 메일함을 확인해주세요.");
            }
        } catch (error: any) {
            setMessage(error.message);
        } finally {
    ...
    return (
        <div className="flex flex-col items-center justify-center p-6 bg-white border border-gray-200 rounded-2xl shadow-sm text-center">
            ...
            <div className="relative w-full my-4">
                ...
                <div className="relative flex justify-center text-xs uppercase"><span className="bg-white px-2 text-gray-500">또는 이메일로 로그인</span></div>
            </div>
            ...
            <form onSubmit={handleEmailLogin} className="w-full space-y-3">
                ...
                <button
                    type="submit"
                    disabled={loading}
                    className="w-full py-2.5 px-4 bg-gray-900 text-white rounded-xl font-bold hover:bg-gray-800 transition-colors flex items-center justify-center gap-2 shadow-sm"
                >
                    {loading ? <Loader2 className="animate-spin w-4 h-4" /> : <div className="w-4 h-4" />}
                    이메일로 로그인 링크 받기
                </button>
            </form>

            {message && (
                <div className={`mt-4 p-3 rounded-lg text-xs text-left ${message.includes('전송') ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-600 border border-red-200'}`}>
                    {message}
                </div>
            )}
        </div>
